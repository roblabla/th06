...
def run_msiextract_win32(msi_file_path: Path, output_dir: Path) -> int:
    return subprocess.check_call(
        ["msiexec", "/a", msi_file_path, "/qb", f"TARGETDIR={output_dir}"],
        cwd=output_dir,
    )
...
def download_requirements(dl_cache_path, steps):
    requirements = [
        ...
        },
        {
            ...
            "filename": "cygwin-setup-x86_64-2.874.exe",
...
def main(args: Namespace) -> int:
    ...
    if not args.download:
        ...
        cygwin_installer_path = dl_cache_path / "cygwin-setup-x86_64-2.874.exe"
