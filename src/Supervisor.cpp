...
{
    for (int i = 0; i < sizeof(s->pbg3Archives) / sizeof(s->pbg3Archives[0]); i++)
    {
        s->pbg3Archives[i] = NULL;
    }

    g_Pbg3Archives = s->pbg3Archives;
    s->LoadPbg3(IN_PBG3_INDEX, TH_IN_DAT_FILE);
    if (s->pbg3Archives != NULL)
    {
        return ZUN_ERROR;
    }
    ...
    i32 res = g_Supervisor.LoadPbg3(MD_PBG3_INDEX, TH_MD_DAT_FILE);
